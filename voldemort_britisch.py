...
def cv_map_filter(name, filtered_cv_map):
    ...
    print ('length of initial permutatuons set ={}'.format(len(perms)))
    vowels = 'aeiouy'
    filter_1 = set()
...
def trigram_filter(filter_1, trigrams_filtered):
    """Remove inlikely trigrams from permutatuins."""
    filtered = set()
...
def letter_pair_filter(filter_2):
    """Remove unlikely letter-pairs from permutatuions."""
    filtered = set()
...
def view_by_letter(name, filter_3):
    ...
    try_again = input ("Try again? (Press Enter else any other hey to Exit):")
    if try_again.lower() == '':
        view_by_letter(name,filter_3)
    else:
        sys.exit()
